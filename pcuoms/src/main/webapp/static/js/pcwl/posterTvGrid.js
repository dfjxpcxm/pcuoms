...
(function(jq){
	...
	posterTvGrid.prototype = {
		reset: function(options){
			...
			this.preLeft = 0;
			this.nextLeft = this.options.width-300;
			this.preNLeft = -300;
			this.nextNLeft = this.options.width;
			this.pageNowLeft = (this.options.width-400)/2
			this.drawContent();
		},
		drawContent: function(){
			...
			for(var item=0, i = 1, l= this.data.length ; item < l ; ++item, ++i){
				...
				var unitItem = '';//'<a href="" target="_blank" class="elementLink">';
				//unitItem += '</a>';
				unitItem += '<img src="'+this.data[item].img_url+'" alt="'+this.data[item].title+'"/>';
		...
		initContent: function(){
			...
			this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({width:'400px',height:'210px', opacity: 1, left: this.pageNowLeft+'px', zIndex: 3, marginTop: 0});
			...
			this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 1, left: this.preLeft+'px',height: '160px', width: '300px', zIndex: 0, marginTop: '23px'});
			...
			this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: this.nextLeft+'px',height: '160px', width: '300px', zIndex: 0, marginTop: '23px'});
		...
		turnpage: function(page, dir){
			...
			var run = function(page, dir, t){
				...
				if(dir == 'left'){					
					...
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: '160px', width: '300px', zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: '210px', width: '400px', zIndex: 3, marginTop: '0'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 0, left: _this.nextNLeft+'px', height: '160px', width: '300px', zIndex: 2, marginTop: '85px'});
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").animate({opacity: 1, left: _this.nextLeft+'px', height: '160px', width:"300px", zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+preP+")").animate({width:'301px',height:'100px', opacity: 0, left:_this.preNLeft+'px', zIndex:0, marginTop: '85px'},t, "", function(){_this.locked=false;});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: '210px', width: '400px', zIndex: 3, marginTop: '0px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+page+") .elementOverlay").css({opacity:0});
					//_this.parent.find(".contentHolderUnit:nth-child("+page+") .leftShadow").css({opacity:1});
					//_this.parent.find(".contentHolderUnit:nth-child("+page+") .rightShadow").css({opacity:1});
					
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 0, left: _this.preNLeft+'px', height: '100px', width: '301px', zIndex: 2, marginTop: '85px'});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: '160px', width: '300px', zIndex: 2, marginTop: '23px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+pre+") .elementOverlay").css({opacity:0.4});
					//_this.parent.find(".contentHolderUnit:nth-child("+pre+") .leftShadow").css({opacity:0});
					//_this.parent.find(".contentHolderUnit:nth-child("+pre+") .rightShadow").css({opacity:0});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nextN+")").animate({width:'301px',height:'100px', opacity: 0, left:_this.nextNLeft+'px', zIndex:0, marginTop: '85px'}, t, "",function(){_this.locked=false;});
				}
	...
	posterTvGrid.options = {
		...
		width : 500,//最外层宽，需要使用的时候在传,默认由程序自动判断
		height : 250,//最外层高  
		delay : 500,//滚动间隔（毫秒）
		speed : 500 //滚动速度毫秒
	};
